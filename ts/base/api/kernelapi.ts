...
export default class KernelApi {
  ...
  public async register(
    params: model.RegisterType
  ): Promise<model.ResultType<any>> {
    let res = await this._restRequest("Register", params);
    if (res.success) {
      this.accessToken = res.data.accessToken;
    }
    return res;
   ...
   * @returns 异步结果
   */
  public async dataProxy(
    req: model.DataProxyType
  ): Promise<model.ResultType<any>> {
          ...
          if (connectionId && connectionId.length > 0) {
            if (this.onlineIds.length < 1) {
              ...
              if (res.target === "Online") {
                ...
                this.onlineIds = this.onlineIds.filter(
                  (i) => i != connectionId
                );
              }
              this.onlineNotity.changCallback();
  ...
  private async _restRequest(
    ...
    return new Promise((resolve, reject) => {
      uni.request({
        ...
        url: "https://asset.orginone.cn/orginone/kernel/rest/" + methodName,
        header: {
